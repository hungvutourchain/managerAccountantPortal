...
export class DepartmentComponent implements OnInit {
  ...
  Getlist(nation) {
    //GetdataByGolive();
    this.dbService.GetDepartment(nation).subscribe((rs: any) => {
      this.lsobject = rs;
      this._oderby()
    });
  };
  ...
  objectValue(action, object) {
    if (action === 'delete') {
      if (confirm('Are you sure to delete?')) {
        this.dbService.RemoveDepartment(object).subscribe((rs: any) => {
      ...
      this.dbService.AddDepartment(temp).subscribe((rs: any) => {
        if (rs) {
          this.Getlist(this.user.nation)
          this.out.emit();   
          this.object = {};
      ...
      this.dbService.UpdateDepartment(object).subscribe((rs: any) => {
        this.Getlist(this.user.nation)
        this.out.emit();
        this.object = {};
        this.openDialog = false

...
export class ClassViewComponent implements OnInit {
  ...
  Getlist(nation) {
    ...
    this.dbService.GetClass(nation).subscribe((rs: any) => {
      this.lsobject = rs;      
      this._oderby()
    });
  };
  ...
  objectValue(action, object) {
    if (action === 'delete') {
      if (confirm('Are you sure to delete?')) {
        this.dbService.RemoveClass(object).subscribe((rs: any) => {
      ...
      this.dbService.AddClass(temp).subscribe((rs: any) => {
        if (rs) {
          this.Getlist(this.user.nation)
          this.out.emit();   
          this.object = {};
      ...
      this.dbService.UpdateClass(object).subscribe((rs: any) => {
        this.Getlist(this.user.nation)
        this.out.emit();
        this.object = {};
        this.openDialog = false
